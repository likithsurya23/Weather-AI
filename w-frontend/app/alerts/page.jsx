            ...
            <div className="lg:col-span-8 space-y-2.5 sm:space-y-3.5">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] text-slate-400 font-medium">
                  {sortedNews.length} {t('alerts.reports', 'reports')}
                </span>
              </div>
              ...
              <div className="space-y-2 sm:space-y-3">
                {loading ? (
                        ...
                        <div className="w-full sm:w-40 lg:w-44 h-28 sm:h-28 rounded-md sm:rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-800 relative shrink-0">
                          ...
                          {article.isBreaking && (
                            <span className="absolute top-1.5 left-1.5 px-1 py-0.5 rounded-xs bg-red-600 text-white text-[8px] sm:text-[9px] font-bold uppercase tracking-wider shadow-xs z-10">
                              {t('alerts.breakingBadge', 'BREAKING')}
                            </span>
                          )}
                        </div>
                        ...
                        <div className="flex-1 flex flex-col justify-between min-w-0">
                          ...
                          <div className="mt-1.5 pt-1.5 border-t border-slate-100 flex flex-wrap items-center justify-between gap-1 text-[9px] sm:text-[10px]">
                            ...
                            <div className="text-blue-600 font-semibold text-[10px] sm:text-[11px] flex items-center gap-0.5 group-hover:underline">
                              <span>{t('alerts.readFullCoverage', 'Read More')}</span>
                  ...
                  <div className="bg-white rounded-lg sm:rounded-xl p-8 text-center border border-slate-200/80">
                    <p className="text-xs sm:text-sm font-semibold text-slate-600">{t('alerts.noReportsFound', 'No disaster reports found.')}</p>
                    <p className="text-[11px] text-slate-400 mt-1">{t('alerts.noReportsHint', 'Try selecting another hazard category or clearing your search.')}</p>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center justify-center gap-1 pt-2 sm:pt-2.5">
                <button
                  ...
                  title={t('alerts.previousPage', 'Previous Page')}
                >
                ...
                <button
                  ...
                  className="p-1 sm:p-1.5 rounded-md bg-white border border-slate-200 hover:bg-slate-50 text-slate-600 disabled:opacity-40 transition-colors cursor-pointer shadow-xs"
                  title={t('alerts.nextPage', 'Next Page')}
                >
            ...
            <div className="lg:col-span-4 space-y-2.5 sm:space-y-3">
              ...
              <div className="bg-white rounded-lg sm:rounded-xl p-2.5 sm:p-3 border border-slate-200/80 shadow-xs">
                <div className="flex items-center gap-1.5 pb-1.5 border-b border-slate-100">
                  ...
                  <h3 className="text-xs sm:text-sm font-bold text-slate-900">{t('alerts.trendingTopics', 'Trending Topics')}</h3>
                </div>
              ...
              <div className="bg-white rounded-lg sm:rounded-xl p-2.5 sm:p-3 border border-slate-200/80 shadow-xs space-y-1.5">
                <div className="flex items-center gap-1.5">
                  ...
                  <h3 className="text-xs sm:text-sm font-bold text-slate-900">{t('alerts.stayUpdated', 'Stay Updated')}</h3>
                </div>

                <p className="text-[10px] sm:text-[11px] text-slate-500 leading-relaxed font-normal">
                  {t('alerts.stayUpdatedDesc', 'Get instant alerts for major natural disasters in your preferred locations.')}
                </p>
                ...
                >
                  {t('alerts.setAlertPrefs', 'Set Alert Preferences')}
                </button>
              </div>
            ...
            <div className="p-4 sm:p-5 space-y-3">
              ...
              <div className="flex items-center justify-between pt-1">
                {selectedArticle.url ? (
                  ...
                  >
                    <span>{t('alerts.readFullSource', 'Read Full Source')}</span>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in-95">
            <div className="p-3.5 sm:p-4 border-b border-slate-100 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-slate-900">{t('alerts.preferences', 'News Preferences')}</h3>
              </div>
              <button
            ...
            <div className="p-3.5 sm:p-4 space-y-3">
              <p className="text-[11px] text-slate-500">
                {t('alerts.prefModalDesc', 'Customize your disaster feed alerts and notifications.')}
              </p>

              <div className="space-y-2">
                {[
                  { key: 'breakingNews', label: t('alerts.breakingNewsAlerts', 'Breaking News Alerts'), desc: t('alerts.breakingNewsDesc', 'Notify immediately on critical red-alert disasters') },
                  { key: 'localAlerts', label: t('alerts.localRegionAlerts', 'Local Region Alerts'), desc: t('alerts.localRegionDesc', 'Prioritize events near your active weather city') },
                  { key: 'earthquakes', label: t('alerts.earthquakeMonitoring', 'Earthquake Monitoring'), desc: t('alerts.earthquakeDesc', 'Seismic detection magnitude >= 4.5') },
                  { key: 'cyclones', label: t('alerts.cycloneWarnings', 'Cyclone & Hurricane Warnings'), desc: t('alerts.cycloneDesc', 'Tropical storms and coastal advisories') },
                  { key: 'floods', label: t('alerts.floodAlerts', 'Flood & Precipitation Alerts'), desc: t('alerts.floodDesc', 'Heavy monsoon and river level advisories') }
                ].map((item) => (
            ...
            <div className="p-3 px-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
              {preferencesSaved ? (
                <span className="text-[11px] font-bold text-emerald-600 flex items-center gap-1">
                  <Check className="w-3.5 h-3.5" /> {t('alerts.saved', 'Saved!')}
                </span>
              ) : <div />}
