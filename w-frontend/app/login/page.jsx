...
function LoginForm() {
  ...
  const { login, isAuthenticated, authLoading, t } = useApp();
    ...
    <div className="w-full max-w-md bg-white/85 dark:bg-slate-900/85 backdrop-blur-xl border border-slate-200/80 dark:border-slate-800/80 rounded-2xl sm:rounded-3xl p-4 sm:p-8 shadow-xl sm:shadow-2xl transition-all relative">
      ...
      <div className="text-center mb-4 sm:mb-6 pt-1 sm:pt-2">
        ...
        <p className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 mt-0.5 sm:mt-1">
          {t('auth.loginSubtitle', 'Login to your WeatherWise account')}
        </p>
      </div>
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              className="w-full pl-9 sm:pl-10 pr-3 sm:pr-4 py-2 sm:py-3 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 rounded-xl sm:rounded-2xl text-xs sm:text-sm text-slate-900 dark:white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/40"
            />
